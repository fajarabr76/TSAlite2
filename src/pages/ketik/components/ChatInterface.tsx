...
export default function ChatInterface({ 
  ...
  const renderMessageContent = (text: string) => {
    const scenarioImages = scenario.images || [];
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white dark:bg-[#000000] overflow-hidden relative">
      {/* 1. iOS Header */}
      <div className="px-4 py-3 flex items-center justify-between border-b border-gray-200/50 dark:border-white/10 shrink-0 bg-white/80 dark:bg-[#1C1C1E]/80 backdrop-blur-xl w-full z-50 relative">
        ...
        <div className="flex flex-col items-center justify-center w-2/4">
            <h1 className="font-bold text-gray-900 dark:text-white text-sm sm:text-base truncate max-w-full text-center">
                {config.identity.name}
            </h1>
            {!isReviewMode ? (
                <div className="flex items-center gap-1.5 text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 font-medium">
                ...
                <span className="text-[10px] text-gray-500 dark:text-gray-400">Review Mode</span>
            )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto z-10 scroll-smooth custom-scrollbar flex flex-col p-4 space-y-2 bg-white dark:bg-[#000000]">
            ...
            {messages.map((msg, index) => {
               if (msg.sender === 'system') {
                   return (
                       ...
                       >
                           <p className="text-[10px] font-medium text-gray-400 dark:text-gray-500 text-center uppercase tracking-wide">
                               {msg.text}
                           </p>
                       </motion.div>
                   );
                ...
                >
                    <div 
                        ...
                        ${isAgent 
                            ...
                            : 'bg-[#E9E9EB] dark:bg-[#262628] text-black dark:text-white rounded-[20px] rounded-bl-sm'
                        }`}
                    >
                        <div>
                            {renderMessageContent(msg.text)}
                        </div>
                        {/* Timestamp & Status (Hidden by default in iOS, but useful here) */}
                        <div className={`text-[9px] flex items-center justify-end gap-1 mt-1 opacity-70 ${isAgent ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`}>
        ...
        {isLoading && (
             ...
             >
                <div className="bg-[#E9E9EB] dark:bg-[#262628] rounded-[20px] rounded-bl-sm px-4 py-3">
                    <div className="flex space-x-1">
                        <motion.div animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.6 }} className="w-1.5 h-1.5 bg-gray-400 dark:bg-gray-500 rounded-full" />
                        <motion.div animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.6, delay: 0.2 }} className="w-1.5 h-1.5 bg-gray-400 dark:bg-gray-500 rounded-full" />
                        <motion.div animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.6, delay: 0.4 }} className="w-1.5 h-1.5 bg-gray-400 dark:bg-gray-500 rounded-full" />
                    </div>
                </div>
            </motion.div>
        )}
      ...
      {/* Input Area */}
      {!isReviewMode ? (
        <div className="p-3 bg-[#F2F2F7] dark:bg-[#1C1C1E] border-t border-gray-200/50 dark:border-white/10 z-40 shrink-0 backdrop-blur-xl">
          {/* Template Button - More Visible */}
          <div className="flex justify-center mb-2">
             <button 
                onClick={applyTemplate}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-[#2C2C2E] rounded-full shadow-sm border border-gray-200 dark:border-white/5 text-[11px] font-bold text-blue-500 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-all"
             >
          ...
          <div className="max-w-4xl mx-auto flex items-end gap-3">
              <div className="flex-1 bg-white dark:bg-[#2C2C2E] rounded-[20px] border border-gray-300 dark:border-transparent flex items-center px-4 py-1 focus-within:border-blue-500 transition-colors">
                  <textarea
                      ref={textareaRef}
                      value={inputText}
                      ...
                      className="w-full bg-transparent border-none outline-none resize-none max-h-32 min-h-[36px] py-2 text-[15px] text-gray-900 dark:text-white placeholder-gray-400"
                      rows={1}
                  />
              </div>
              <motion.button 
        ...
        <div className="p-6 bg-[#F2F2F7] dark:bg-[#1C1C1E] border-t border-gray-200/50 dark:border-white/10 z-40 shrink-0 text-center">
