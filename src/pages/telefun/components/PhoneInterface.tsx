...
export const PhoneInterface: React.FC<PhoneInterfaceProps> = ({ 
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row h-full w-full bg-gray-100 dark:bg-[#111827] relative text-gray-900 dark:text-white overflow-hidden font-sans transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col relative z-10 w-full h-full overflow-y-auto custom-scrollbar">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center p-4 md:p-6 shrink-0 gap-4">
            <div className="flex items-center gap-3 bg-white/50 dark:bg-gray-800/50 px-4 py-2 rounded-full backdrop-blur-sm border border-gray-200 dark:border-white/5 shadow-sm self-start md:self-auto">
                ...
                <span className="text-sm font-medium tracking-wide text-gray-800 dark:text-gray-200 font-mono">{formatTime(callDuration)}</span>
            </div>

            {/* Status Pill - Moved to Top */}
            <div className={`px-4 py-2 rounded-xl border transition-all duration-300 ${statusBg} ${statusBorder} shadow-sm backdrop-blur-md self-stretch md:self-auto flex items-center justify-center`}>
                <p className={`text-xs md:text-sm font-semibold ${statusTextColor} animate-pulse`}>
                    {statusText}
                </p>
            </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 md:px-12 w-full max-w-4xl mx-auto py-4">
            ...
            <div className="relative mb-6">
                ...
                {isRinging && (
                    ...
                    <div className="absolute inset-0 rounded-full bg-white/20 animate-ping delay-150"></div>
                    </>
                )}
                ...
                <div className={`absolute inset-0 rounded-full bg-white/10 scale-110 transition-transform duration-300 ${isAiSpeaking && !isOnHold ? 'animate-ping opacity-30' : 'opacity-0'}`}></div>
                
                <div className="w-32 h-32 md:w-56 md:h-56 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-800 shadow-2xl border border-gray-300 dark:border-gray-700/50 relative z-10">
                    <img src={`https://picsum.photos/seed/${config.identity.name}/500`} alt="Avatar" className={`w-full h-full object-cover transition-all ${isOnHold ? 'grayscale blur-sm' : 'grayscale-[0.2]'}`} />
                </div>
            </div>

            {/* Info Text */}
            <h1 className="text-2xl md:text-4xl font-bold mb-2 tracking-tight text-gray-900 dark:text-white text-center">{config.identity.name}</h1>
            <p className="text-sm md:text-lg text-gray-600 dark:text-gray-400 mb-6 font-medium text-center">{config.identity.phone} • {config.identity.city}</p>

            {/* AGENT VOICE INDICATOR */}
            {!isOnHold && connectionState === "Tersambung" && (
                <div className="w-full max-w-xs md:max-w-sm mb-6 flex flex-col gap-1">
                    ...
                    <div className="h-2 bg-gray-800 rounded-full overflow-hidden border border-gray-700 relative">
                        {/* Background Markers */}
                        <div className="absolute left-[33%] top-0 bottom-0 w-px bg-gray-600/50"></div>
                        <div className="absolute left-[66%] top-0 bottom-0 w-px bg-gray-600/50"></div>
      ...
      <div className="
        shrink-0 z-20 
        bg-white/80 dark:bg-gray-900/50 backdrop-blur-md border-t border-gray-200 dark:border-white/5 md:border-t-0 md:border-l
        flex 
      ...
      ">
         ...
         {/* Hold Button */}
          <div className="flex flex-col items-center gap-2">
            <button 
                onClick={toggleHold}
                disabled={isRinging}
                className={`p-4 md:p-5 rounded-full transition-all duration-200 border shadow-lg ${isOnHold ? 'bg-yellow-400 text-black border-yellow-400 hover:bg-yellow-300' : 'bg-gray-100 dark:bg-gray-800/80 text-gray-800 dark:text-white border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed'}`}
            ...
            <span className="text-[10px] uppercase font-bold tracking-wider text-gray-500 hidden md:block">Hold</span>
         </div>
         ...
         <div className="flex flex-col items-center gap-2">
            <button 
                ...
                className={`p-4 md:p-5 rounded-full transition-all duration-200 border shadow-lg ${isMuted ? 'bg-gray-800 dark:bg-white text-white dark:text-black border-gray-800 dark:border-white hover:bg-gray-700 dark:hover:bg-gray-200' : 'bg-gray-100 dark:bg-gray-800/80 text-gray-800 dark:text-white border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-50'}`}
            ...
            <span className="text-[10px] uppercase font-bold tracking-wider text-gray-500 hidden md:block">Mic</span>
         </div>
