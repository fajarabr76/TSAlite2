...
import { X, Plus, Check, Edit2, Trash2, Image as ImageIcon, User, Settings, FileText, Users, Clock } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, settings, onSave }) => {
  ...
  // removed newScenarioConsumerType state
  ...
  const [newConsumerDifficulty, setNewConsumerDifficulty] = useState<ConsumerDifficulty>('Medium');
  ...
  const handleEditScenario = (scenario: Scenario) => {
    ...
    } else if ((scenario as any).attachmentImage) {
        // Fallback for old single image data
        images = [(scenario as any).attachmentImage];
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        {/* Modal Header */}
        <div className="px-8 py-6 border-b border-gray-200/50 dark:border-white/5 flex justify-between items-center bg-white/50 dark:bg-[#1C1C1E]/50 backdrop-blur-xl shrink-0">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Pengaturan Simulasi</h2>
          <div className="flex items-center gap-2">
            <button 
                onClick={handleSave}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm transition-all shadow-lg shadow-blue-500/20"
            >
                Simpan Perubahan
            </button>
            <button 
        ...
        <div className="px-8 pt-6 pb-2 shrink-0 bg-white dark:bg-[#1C1C1E]">
          <div className="flex p-1 bg-gray-100 dark:bg-[#2C2C2E] rounded-xl">
            {tabs.map((tab) => (
              <button
                key={tab.id}
              ...
              >
                {activeTab === tab.id && (
                  <motion.div
                    layoutId="activeTab"
                    className="absolute inset-0 bg-white dark:bg-[#3A3A3C] rounded-lg"
        ...
        {/* Modal Body */}
        <div className="flex-1 overflow-y-auto p-8 bg-[#F2F2F7] dark:bg-[#000000]">
          {activeTab === 'scenarios' && (
            <div className="space-y-6">
                  ...
                  >
                      ...
                      <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 leading-relaxed">
                           ...
                           {((scenario.attachmentImages && scenario.attachmentImages.length > 0) || (scenario as any).attachmentImage) && (
                              <div className="mt-2">
                                   <span className="text-[10px] bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2 py-1 rounded-md inline-flex items-center gap-1 font-medium">
                                      ...
                                      {scenario.attachmentImages?.length || 1} Lampiran
                                  </span>
                              </div>
                          )}
                      </div>
              ...
              {!isAddingScenario && !editingScenarioId ? (
                <button 
                  ...
                  className="w-full py-4 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400 font-bold hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 transition-all flex items-center justify-center gap-2 shadow-sm bg-white dark:bg-[#1C1C1E]"
                >
                ...
                <div id="scenario-form" className="bg-white dark:bg-[#1C1C1E] rounded-2xl border border-gray-200 dark:border-white/10 shadow-lg overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-200 dark:border-white/10 bg-gray-50/50 dark:bg-[#2C2C2E]/50 flex justify-between items-center">
                    ...
                    {!isNewCategoryInput ? (
                      ...
                      <div className="flex gap-2">
                        ...
                        <button 
                          ...
                          className="px-4 text-xs text-red-500 font-bold hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-all"
                        >
                          Batal
                        </button>
                      </div>
                  ...
                  <div className="col-span-2">
                    <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2">Script / Prompt Tambahan (Opsional)</label>
                    <textarea 
                      className="w-full rounded-xl border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#2C2C2E] p-3 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none resize-none"
                      rows={2}
                      placeholder="Instruksi khusus untuk AI dalam skenario ini..."
                  ...
                  <div className="col-span-2">
                    ...
                    <div className="flex items-center justify-center w-full">
                        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 dark:hover:bg-[#2C2C2E] dark:bg-[#1C1C1E] hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 transition-all">
                        ...
                        {newScenarioImages.map((img, index) => (
                          <div key={index} className="relative w-20 h-20 shrink-0 group">
                            <img 
                              src={img} 
                              alt={`Preview ${index}`} 
                              className="w-full h-full object-cover rounded-xl border border-gray-200 dark:border-white/10" 
                            />
                            <button 
                  ...
                  <div className="col-span-2 flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-white/10">
                    <button 
                      onClick={resetScenarioForm}
                      className="px-6 py-2.5 rounded-xl text-gray-600 dark:text-gray-400 font-bold hover:bg-gray-100 dark:hover:bg-[#2C2C2E] transition-all"
                    >
                      Batal
                    </button>
                    <button 
          ...
          {activeTab === 'consumers' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Random Option */}
                <div 
                  onClick={() => setLocalSettings(prev => ({ ...prev, globalConsumerTypeId: 'random' }))}
                  className={`cursor-pointer border p-4 rounded-xl shadow-sm group transition-all relative ${localSettings.globalConsumerTypeId === 'random' ? 'bg-blue-50 border-blue-500 ring-2 ring-blue-500 dark:bg-blue-900/20 dark:border-blue-400' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-blue-300'}`}
                >
                    <div className="flex justify-between items-start mb-2">
                      <h4 className={`font-bold flex items-center gap-2 ${localSettings.globalConsumerTypeId === 'random' ? 'text-blue-700 dark:text-blue-300' : 'text-gray-800 dark:text-gray-100'}`}>
                        <span className={`w-2 h-2 rounded-full ${localSettings.globalConsumerTypeId === 'random' ? 'bg-blue-600 animate-pulse' : 'bg-gray-400'}`}></span>
                        Acak (Random)
                      </h4>
                    ...
                    <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">Sistem akan memilih tipe konsumen secara acak untuk setiap sesi simulasi.</p>
                </div>

                {localSettings.consumerTypes.map(c => (
                  <div 
                    key={c.id} 
                    onClick={() => setLocalSettings(prev => ({ ...prev, globalConsumerTypeId: c.id }))}
                    className={`cursor-pointer border p-4 rounded-xl shadow-sm group transition-all relative ${localSettings.globalConsumerTypeId === c.id ? 'bg-blue-50 border-blue-500 ring-2 ring-blue-500 dark:bg-blue-900/20 dark:border-blue-400' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-blue-300'} ${editingConsumerId === c.id ? 'ring-2 ring-yellow-400 bg-yellow-50 dark:bg-yellow-900/10' : ''}`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      <h4 className={`font-bold flex items-center gap-2 pr-8 ${localSettings.globalConsumerTypeId === c.id ? 'text-blue-700 dark:text-blue-300' : 'text-gray-800 dark:text-gray-100'}`}>
                        <span className={`w-2 h-2 rounded-full ${localSettings.globalConsumerTypeId === c.id ? 'bg-blue-600' : 'bg-gray-400'}`}></span>
                        {c.name}
                      </h4>
                      <div className="flex items-center gap-2">
                        {localSettings.globalConsumerTypeId === c.id && (
                        ...
                        <button 
                          ...
                          className="text-gray-400 hover:text-blue-600 transition-colors p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg"
                        ...
                        <button 
                          ...
                          className="text-gray-400 hover:text-red-600 transition-colors p-1 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"
                    ...
                    <div className="flex gap-2 mb-2">
                        {c.difficulty && (
                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium border ${
                                c.difficulty === 'Hard' ? 'bg-red-100 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800' :
                                c.difficulty === 'Medium' ? 'bg-yellow-100 text-yellow-700 border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 dark:border-yellow-800' :
                        ...
                        {c.tone && (
                            <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-gray-100 text-gray-600 border border-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600">
                                {c.tone}
                            </span>
                        )}
                    </div>
                    <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed line-clamp-3">{c.description}</p>
                  </div>
                ))}
              </div>
              ...
              {!isAddingConsumer && !editingConsumerId ? (
                <button 
                  ...
                  className="w-full py-4 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400 font-bold hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 transition-all flex items-center justify-center gap-2"
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2 flex justify-end gap-2">
                      <button 
                        ...
                        className={`${editingConsumerId ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-blue-600 hover:bg-blue-700'} text-white px-8 py-2.5 rounded-lg font-bold shadow-md transition-all active:scale-95 disabled:opacity-50`}
                      >
          ...
          {activeTab === 'identity' && (
            <div className="space-y-6">
              <div className="bg-gray-50/50 dark:bg-white/5 px-4 py-3 rounded-2xl border border-gray-200 dark:border-white/10 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                      <h3 className="text-sm font-black text-gray-900 dark:text-white tracking-tight uppercase">Identitas Konsumen</h3>
                      <div className="h-4 w-px bg-gray-300 dark:bg-gray-700 hidden sm:block"></div>
                      <p className="text-xs text-gray-500 dark:text-gray-400 hidden md:block">Atur identitas pengirim email simulasi.</p>
                  </div>
              </div>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="col-span-2">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h4 className="font-bold text-gray-900 dark:text-white">Profil Pengirim</h4>
                                <p className="text-xs text-gray-500 dark:text-gray-400">Informasi ini akan muncul sebagai pengirim email.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input 
                            type="text" 
                            className="w-full rounded-xl border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#2C2C2E] p-3 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                            placeholder="Contoh: Budi Santoso"
                            value={customSenderName}
                            onChange={(e) => setCustomSenderName(e.target.value)}
                        />
                        <p className="text-[10px] text-gray-400 mt-1.5 ml-1">Muncul di header email (From: ...)</p>
                    </div>
                    ...
                    <div>
                        ...
                        />
                        <p className="text-[10px] text-gray-400 mt-1.5 ml-1">Muncul di akhir email (Salam, ...)</p>
                    </div>
          ...
          {activeTab === 'system' && (
            <div className="space-y-6">
              <div className="bg-gray-50/50 dark:bg-white/5 px-4 py-3 rounded-2xl border border-gray-200 dark:border-white/10 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                      <h3 className="text-sm font-black text-gray-900 dark:text-white tracking-tight uppercase">Pengaturan Sistem</h3>
                      <div className="h-4 w-px bg-gray-300 dark:bg-gray-700 hidden sm:block"></div>
                      <p className="text-xs text-gray-500 dark:text-gray-400 hidden md:block">Atur model AI dan fitur sistem lainnya.</p>
                  </div>
              </div>

              <div className="bg-white dark:bg-[#1C1C1E] rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm overflow-hidden p-6">
                  <div className="space-y-6">
                      <div>
                          <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-3">Model AI</label>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {AI_MODELS.map(model => (
                                  <div 
                                      key={model.id}
                                      onClick={() => setSelectedModel(model.id)}
                                      className={`cursor-pointer border p-4 rounded-xl transition-all relative ${
                                          selectedModel === model.id 
                                              ? 'bg-blue-50 border-blue-500 ring-2 ring-blue-500 dark:bg-blue-900/20 dark:border-blue-400' 
                                              : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-blue-300'
                                      }`}
                                  >
                                      <div className="flex items-center justify-between mb-1">
                                          <h4 className={`font-bold text-sm ${selectedModel === model.id ? 'text-blue-700 dark:text-blue-300' : 'text-gray-900 dark:text-white'}`}>
                                              {model.name}
                                          </h4>
                                          {selectedModel === model.id && <Check className="w-4 h-4 text-blue-600 dark:text-blue-400" />}
                                      </div>
              ...
              <div className="pt-6 mt-6 border-t border-gray-200 dark:border-white/10">
                <button
                    ...
                    className="w-full py-3 text-red-500 text-sm font-bold hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-all border border-transparent hover:border-red-200 dark:hover:border-red-500/20 flex items-center justify-center gap-2"
                >
                    <Trash2 className="w-4 h-4" />
